...
@section('content')
    <div class="card">
        <div class="text-center">
            <div class="card-body">
                <img src="@if($data->profile_image!=' ' &&  $data->profile_image!='0'){{$data->profile_image}}@else https://image.flaticon.com/icons/png/512/73/73199.png @endif" alt="{{$data->username}} image" class="user-profile">
            </div>
            ...
            <div class="card-body">
                ...
                <br>
                <h6 class="card-subtitle text-muted"><span class="field-label">Languages Known:</span>{{$data->languages_known ?? '-'}}</h6>
                <br>
                <h6 class="card-subtitle text-muted"><span class="field-label">Date of Birth:</span>{{date('m-d-Y', strtotime($data->date_of_birth))}}</h6>
                <br>
            ...
            <?php $address = implode(
                ...
                @if($data->role_id==0 || $data->role_id==3)
                    ...
                    <br>
                    <h6 class="card-subtitle text-muted"><span class="field-label">Address:</span>{{$address ?? '-'}}</h6>
                    <br>
                @elseif($data->role_id==1 || $data->role_id==4)
                    <h6 class="card-subtitle text-muted"><span class="field-label">Mailing Address:</span>{{$address ?? '-'}}</h6>
                    <br>
                @elseif($data->role_id==2)
                    <h6 class="card-subtitle text-muted"><span class="field-label">Office Number:</span>{{$data->work ?? '-'}}</h6>
                    <br>
                ...
                <div class="text-center">
                    @if($data->facebook_url!='0' && strlen($data->facebook_url) > 0)
                        <a href="{{$data->facebook_url}}" data-href="{{$data->facebook_url}}" target="_blank" class="btn btn-social-icon parse-link-href mr-1 mb-1 btn-outline-facebook">
                            <span class="la la-facebook"></span>
                        </a>
                    @endif
                    @if($data->linkedin_url!='0' && strlen($data->linkedin_url) > 0)
                        <a href="{{$data->linkedin_url}}" data-href="{{$data->linkedin_url}}" target="_blank" class="btn btn-social-icon parse-link-href mb-1 btn-outline-linkedin">
                            <span class="la la-linkedin font-medium-4"></span>
                        </a>
                    @endif
                    @if($data->instagram_url!='0' && strlen($data->instagram_url) > 0)
                        <a href="{{$data->instagram_url}}" data-href="{{$data->instagram_url}}" target="_blank" class="btn btn-social-icon parse-link-href mb-1 btn-outline-linkedin">
                            <span class="la la-instagram font-medium-4"></span>
                        </a>
                    @endif
                    @if($data->twitter_url!='0' && strlen($data->twitter_url) > 0)
                        <a href="{{$data->twitter_url}}" data-href="{{$data->twitter_url}}" target="_blank" class="btn btn-social-icon parse-link-href mb-1 btn-outline-linkedin">
                ...
                @endif
                @if($data->is_verified!=1)
                    <a style="float:right" class="btn btn-default btn-primary btn-block" href="{{BASE_URL}}admin/verify_profile/{{$data->id}}">
        ...
        <div class="card-body">
            ...
            @if(isset($data->phone))
                @if($data->otp_verified == 0)
                    <a style="float:left" class="btn btn-default btn-success" href="{{BASE_URL}}admin/verify_mobile/{{$data->id}}/1"><span style="height:29px;width: 10px">Click here to Verify Phone Number</span></a>
                @elseif($data->otp_verified == 1)
                    <a style="float:left" class="btn btn-default btn-danger" href="{{BASE_URL}}admin/verify_mobile/{{$data->id}}/0"><span style="height:29px">Click here to Unverify Phone Number</span></a>
                @endif
            @endif
        </div>
        @php
                                ...
                                <div class="col-md-4">
                                    <figure class="card card-img-top border-grey border-lighten-2" itemprop="associatedMedia" itemscope="" itemtype="http://schema.org/ImageObject">
                                        ...
                                        @elseif (\Illuminate\Support\Str::endsWith(strtolower($d->document_url), '.heic'))
                                            ...
                                            <img data-enlargable class="gallery-thumbnail card-img-top heic-image" src="{{$d->document_url}}" itemprop="thumbnail" alt="Image description">
                                        @else
                                            <img data-enlargable class="gallery-thumbnail card-img-top" src="{{$d->document_url}}" itemprop="thumbnail" alt="Image description">
                                        @endif
                                        <div class="card-body px-0">
                                            @if($d->status == 0)
                                                <a class="verification-response verification-approved btn btn-default btn-success btn-block" data-id="{{$d->id}}" data-status="1" data-title="{{ucfirst(str_replace("_"," ",$d->document_type))}}">
                                                    <span style="height:30px">Approve This Document</span>
                                                </a>
                                                <a class="verification-response verification-denied btn btn-default btn-danger btn-block" data-id="{{$d->id}}" data-status="-1" data-title="{{ucfirst(str_replace("_"," ",$d->document_type))}}">
