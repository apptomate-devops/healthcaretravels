...
class UsersTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $this->command->info('Encrypting passwords for users count: ' . count($users));
        foreach ($users as $index => $user) {
            $countIndex = $index + 1;
            if ($countIndex % 10 === 0) {
                Log::info('Completed ' . $countIndex . '...');
                $this->command->info('Completed ' . $countIndex . '...');
            }
        ...
        $this->command->info('Done encrypting users passwords');
    }
}
