...
@section('content')
    ...
    <div class="mail-container">
        ...
        <div style="margin: 0; padding: 0; margin-top: 1em">
            ...
            @elseif($mail_to == 'traveller')
                <p>The Homeowner for  {{$property}}   has cancelled the booking {{$booking_id}}  that was Requested for   {{$start_date}} to {{$end_date}}. if you have not checked in and money has been collected for this listing {{APP_BASE_NAME}}  will begin processing a refund.</p>
            @else
                <p>The Homeowner for  {{$property}}   has cancelled the booking {{$booking_id}}  that was Requested for   {{$start_date}} to {{$end_date}}. if you have not checked in and money has been collected for this listing {{APP_BASE_NAME}}  will begin processing a refund.</p>
            @endif
        </div>
